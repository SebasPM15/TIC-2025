	...
	void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
	...
	for(int y=patternPadding+1;y<hG[0]-patternPadding-2;y++)
		for(int x=patternPadding+1;x<wG[0]-patternPadding-2;x++)
		{
			...
			float idepth1=1.0/(*(depthmap_ptr+i));
			float var1 = 1.0/pow(6*(*(depthmap_ptr+i)),2);
			impt->idepth_max=idepth1+sqrt(var1);
			impt->idepth_min=idepth1-sqrt(var1);
			if(impt->idepth_min<0) impt->idepth_min=0;
			if(impt->idepth_max<0) impt->idepth_min=0.00000001;
...
// ======== VERSIÓN FINAL Y ROBUSTA de getDepthMap ========
cv::Mat FullSystem::getDepthMap(FrameHessian* fh) {
    // 1. Prepara la imagen (código original)
    ...
    // 2. Crear nombre de archivo único (para depuración)
    std::stringstream ss;
    ss << "depth_debug/frame_";
    ss.fill('0');
    ss.width(5); // Rellena con ceros, ej: "frame_00001.txt"

    // --- LA CORRECCIÓN ESTÁ AQUÍ ---
    ss << fh->idx; // <--- CORREGIDO: Usamos 'idx' en lugar de 'id_'
    // --- FIN DE LA CORRECCIÓN ---

    ss << ".txt";
    std::string depth_filename = ss.str();


    // 3. Guardar la imagen de entrada (código original)
    imwrite("test.jpg", image); // El servidor leerá este archivo


    // 4. Llamar al servidor (código original)
    std::string command = "curl -X POST  http://127.0.0.1:5000/predict";
    (void)system(command.c_str()); // <--- CORREGIDO: (void) silencia la advertencia


    // 5. Mover el resultado (con 'cp' para mantener el original)
    std::stringstream cp_command;
    cp_command << "cp depthcrfs.txt " << depth_filename;
    (void)system(cp_command.str().c_str()); // <--- CORREGIDO: (void) silencia la advertencia


    // 6. Leer el archivo
    cv::FileStorage fs("depthcrfs.txt", cv::FileStorage::READ); 
    if (!fs.isOpened()) { // <--- CORREGIDO: (Cambiado a isOpened())
        fprintf(stderr, "ERROR: [getDepthMap] No se pudo abrir 'depthcrfs.txt' para el frame %d\n", fh->idx);
        return depth; // Devuelve matriz vacía
    }
    ...
    // 6. ======== ¡EL FILTRO SANITARIO! ========
    //    Aquí limpiamos los 'nan' antes de devolverlos.
    if (depth.empty()) {
        fprintf(stderr, "ERROR: [getDepthMap] El mapa de profundidad para el frame %d está VACÍO.\n", fh->idx);
        return depth;
    }
    ...
    for(int r = 0; r < depth.rows; ++r) {
        for(int c = 0; c < depth.cols; ++c) {
            ...
            if(std::isnan(val) || std::isinf(val)) {
                val = 0.0f; // Reemplazar nan/inf por 0 (un valor seguro)
                nan_count++;
            }
        }
    }
    ...
    if(nan_count > 0) {
        printf("ADVERTENCIA: [getDepthMap] Frame %d: Se encontraron y limpiaron %d valores nan/inf.\n", fh->idx, nan_count);
    } else {
        printf("DEBUG: [getDepthMap] Frame %d: Mapa de profundidad limpio.\n", fh->idx);
    }
    // ======== FIN DEL FILTRO SANITARIO ========

    return depth;
}

// ======== AÑADIDO: Implementación de los Getters de Estado ========

bool FullSystem::isMappingRunning()
{
    // No necesitamos un mutex aquí, leer un bool es atómico.
    // 'runMapping' es una variable miembro privada.
    return runMapping; 
}

void FullSystem::getSlamStateInformatica(SlamPointCloud &pointCloud, SlamPose &pose)
{
    // Limpiar los contenedores de salida
    pointCloud.points.clear();
    pose.poseValid = false;

    // Bloquear los mutexs privados de FullSystem (¡esta es la forma correcta!)
    boost::unique_lock<boost::mutex> lock(mapMutex);
    boost::unique_lock<boost::mutex> crlock(shellPoseMutex);

    // --- 1. Obtener la Nube de Puntos (REAL) ---
    // 'ef' y 'frameHessians' son variables miembro privadas
    pointCloud.points.reserve(ef->nPoints); // Reservar espacio
    for (FrameHessian* kf : frameHessians) {
        if (!kf) continue;
        
        // 'get_worldToCam_evalPT' es un método público de FrameHessian
        SE3 camToWorld = kf->get_worldToCam_evalPT().inverse();
        
        // 'pointHessians' es un vector público de FrameHessian
        for (PointHessian* ph : kf->pointHessians) {
            if (!ph || ph->idepth_scaled < 0) continue;

            // 'u', 'v', 'idepth_scaled' son variables públicas de PointHessian
            Vec3 pt_cam(ph->u, ph->v, 1.0);
            pt_cam = pt_cam * (1.0f / ph->idepth_scaled); 
            Vec3 pt_world = camToWorld * pt_cam; 

            // Filtro sanitario
            if(std::isnan(pt_world[0]) || std::isinf(pt_world[0])) continue;
            if(std::isnan(pt_world[1]) || std::isinf(pt_world[1])) continue;
            if(std::isnan(pt_world[2]) || std::isinf(pt_world[2])) continue;

            pointCloud.points.push_back(pt_world.cast<float>()); // Convertir a float
        }
    }

    // --- 2. Obtener la Pose de Cámara (REAL) ---
    // 'allFrameHistory' es una variable miembro privada
    if(!allFrameHistory.empty())
    {
        FrameShell* lastFrame = allFrameHistory.back();
        // 'poseValid' y 'camToWorld' son variables públicas de FrameShell
        if(lastFrame && lastFrame->poseValid)
        {
            pose.poseValid = true;
            pose.camToWorld = lastFrame->camToWorld.cast<float>(); // Convertir a float
        }
    }
    
    // Los mutexs 'lock' y 'crlock' se liberan automáticamente aquí
}
// ======== FIN DE AÑADIDO ========
