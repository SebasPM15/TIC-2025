...
{
	cv::Mat m = cv::imread(filename, cv::IMREAD_GRAYSCALE); // Nombre moderno
	if(m.rows*m.cols==0)
	{
...
{
	cv::Mat m = cv::imread(filename, cv::IMREAD_COLOR); // Nombre moderno
	if(m.rows*m.cols==0)
	{
		printf("cv::imread could not read image %s! this may segfault. \n", filename.c_str());
		return 0;
...
{
	cv::Mat m = cv::imread(filename, cv::IMREAD_UNCHANGED); // Nombre moderno
	if(m.rows*m.cols==0)
	{
		printf("cv::imread could not read image %s! this may segfault. \n", filename.c_str());
		return 0;
...
{
	cv::Mat m = cv::imdecode(cv::Mat(numBytes,1,CV_8U, data), cv::IMREAD_GRAYSCALE); // Nombre moderno
	if(m.rows*m.cols==0)
	{
